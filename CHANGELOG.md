...
### Follow-up: deposit reminder now opens table assignment reply
- После установки депозита без стола бот не просто напоминает про стол, а сразу открывает `force_reply` для номера стола.
- Это закрывает сценарий, когда следующий ответ улетал в другой pending-flow вместо назначения стола.

### Simplify waiter deposit notification message
- Сообщение в группе официантов теперь показывает только стол и депозит.
- Удалены: ID брони, статус, дата/время, имя гостя, номер телефона, количество гостей, комментарии.
- Уведомление стало компактнее для лучшей читаемости:
  - **Стол с депозитом**
  - **Стол:** #999
  - **Депозит:** 77777 руб.
